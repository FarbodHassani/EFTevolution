	  ...
	  if (sim.out_pk & MASK_PI_K)
		{
			...
			writePowerSpectrum(kbin, power, kscatter, pscatter, occupation, sim.numbins, sim.boxsize, (Real) numpts3d * (Real) numpts3d * 2. * M_PI * M_PI/(Hconf(1.0,fourpiG,cosmo) * Hconf(1.0,fourpiG,cosmo)), filename, "power spectrum of pi_k * H (dimensionless)", a, sim.z_pk[pkcount]);
		}
